...
public:
  ...
  explicit DATReader(std::istream& in): m_in(&in){
    ...
    switch (m_header.fields.dtype){
      case 2: {
        m_data_type = DataType::uint8; break;
      }
      case 5: {
        m_data_type = DataType::float32; break;
      }
      default: {
    ...
    switch (m_data_type){
      case DataType::uint8: {
        m_n_frames = n_payload_bytes/m_header.fields.nitems/m_header.fields.ndim/1;
        if (m_header.fields.nitems*m_header.fields.ndim*1*m_n_frames!=n_payload_bytes){
          throw std::runtime_error("File size does not match header info");
        }
        break;
      }
      case DataType::float32: {
        m_n_frames = n_payload_bytes/m_header.fields.nitems/m_header.fields.ndim/4;
        if (m_header.fields.nitems*m_header.fields.ndim*4*m_n_frames!=n_payload_bytes){
          throw std::runtime_error("File size does not match header info");
        }
        break;
      }
  ...
  const std::vector<std::string>& info() const{return m_info;}
  const int& n_frames() const{return m_n_frames;}

  bool match(const xmol::polymer::ConstAtomSelection& sel) const{
    if (m_data_type!=DataType::float32 ||
        sel.size()!=m_header.fields.nitems ||
        m_header.fields.ndim!=3
        ){
      return false;
    }
  ...
  void set_frame(size_t n, const xmol::polymer::AtomSelection& sel){
    ...
    const std::streamoff frame_begin = float_size*m_header.fields.nitems*m_header.fields.ndim*n;
