...
public:
  ...
  template <typename U = T,
            typename SFINAE2 = std::enable_if_t<std::is_const_v<U>>>
...
template <typename T, typename SFINAE = void> struct SelectionTraits {
  ...
  using container_type =
      std::conditional_t<is_const, const Container<value_type>,
                         Container<value_type>>;
  using selection_element_type =
      std::conditional_t<is_const, const value_type*, value_type*>;

  using on_selection_move_type =
      std::conditional_t<is_const, void (container_type::*)(
                                       SelectionBase<const value_type>&,
                                       SelectionBase<const value_type>&) const,
                         void (container_type::*)(SelectionBase<value_type>&,
                                                  SelectionBase<value_type>&)>;

  using on_selection_copy_type =
      std::conditional_t<is_const, void (container_type::*)(
                                       SelectionBase<const value_type>&) const,
                         void (container_type::*)(SelectionBase<value_type>&)>;

  using on_selection_delete_type =
      std::conditional_t<is_const, void (container_type::*)(
                                       SelectionBase<const value_type>&) const,
                         void (container_type::*)(SelectionBase<value_type>&)>;
};
