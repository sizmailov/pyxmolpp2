...
geom::affine::Transformation3d calc_alignment_impl(const MatrixA& X, const MatrixB& Y) {
  if (X.rows() != Y.rows()) {
    throw geom::GeomError("reference.size(" + std::to_string(X.size()) +
  ...
  Eigen::Matrix3d C = (X.rowwise() - xc).transpose() * (Y.rowwise() - yc);
...
Eigen::Matrix3d calc_inertia_tensor_impl(const CoordMatrix& coords, const MassMatrix& mass) {
  ...
  double yy = x2 + z2;
  double zz = y2 + x2;
  double xy =-(((coords.col(X).array() - center(X)) * (coords.col(Y).array() - center(Y))).array() * mass.array()).sum();
  double xz =-(((coords.col(X).array() - center(X)) * (coords.col(Z).array() - center(Z))).array() * mass.array()).sum();
  double yz =-(((coords.col(Y).array() - center(Y)) * (coords.col(Z).array() - center(Z))).array() * mass.array()).sum();
