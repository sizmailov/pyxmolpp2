...
template<int LEN>
inline std::pair<bool, int> parse_uint_fixed_length(const std::string& line, int pos) noexcept {
  bool success = GSL_LIKELY(line.size()>=pos+LEN);
  if (GSL_UNLIKELY(!success)){return {false,0};}
  int number=0;
#pragma unroll
  for (int i=0;i<LEN;i++){
...
inline std::pair<bool, int> parse_uint(const std::string& line, int pos, int LEN) noexcept {
  switch(LEN){
  case (1): return parse_uint_fixed_length<1>(line,pos);
  case (2): return parse_uint_fixed_length<2>(line,pos);
  case (3): return parse_uint_fixed_length<3>(line,pos);
  case (4): return parse_uint_fixed_length<4>(line,pos);
  case (5): return parse_uint_fixed_length<5>(line,pos);
  case (6): return parse_uint_fixed_length<6>(line,pos);
  case (7): return parse_uint_fixed_length<7>(line,pos);
  case (8): return parse_uint_fixed_length<8>(line,pos);
  case (9): return parse_uint_fixed_length<9>(line,pos);
  default: return {false,0};
  }
};

enum class SpaceStrip{
  NONE,
  LEFT,
  RIGHT,
  LEFT_AND_RIGHT
...
inline std::pair<bool, double> operator()(const std::string& line, int pos) noexcept  {

  if (GSL_UNLIKELY(
      line.size()<pos+WIDTH || (PRECISION>0 && line[pos+WIDTH-PRECISION-1]!='.'))
      ){
    return {false,0};
  }

  int whole = WIDTH-PRECISION;
  if (PRECISION>0){--whole;}
  ...
  auto [success2, fraction_part ] = parse_uint(line,pos+whole+1,precision);
  if (GSL_UNLIKELY(!success2)){return {false,0};}
...
inline namespace functional_objects{
  ...
  constexpr parse_fixed_precision_fn parse_fixed_precision = parse_fixed_precision_fn<WIDTH,PRECISION,STRIP>{};
}
