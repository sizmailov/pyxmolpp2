...
template<int MAX_LENGTH, typename=void>
...
public:
  ...
  explicit ShortAsciiString(const std::string& aName) : m_value(to_uint(aName.c_str())){
  }

  constexpr explicit ShortAsciiString(const char* aName) : m_value(to_uint(aName)) {
  }
  ...
  constexpr uint_type value() const {
    return m_value;
  }
...
private:
  constexpr uint_type static to_uint(const char* aName){
    ...
    return (*aName == '\0')
           ? m_value
           : throw std::runtime_error("ShortName::too_long: len(`"+std::string(origin,MAX_LENGTH)+"`...) > "+std::to_string(MAX_LENGTH));
  }
...
struct hash<xmol::utils::ShortAsciiString<N,T>> {
