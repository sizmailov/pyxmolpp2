...
TEST_F(ConSelTests, invalid_selection_throws_on_use) {
  ...
  EXPECT_THROW(s[0], xmol::utils::dead_selection_access<int_with_parent>);
  EXPECT_THROW(s.count(value), xmol::utils::dead_selection_access<int_with_parent>);
  EXPECT_THROW(s.begin(), xmol::utils::dead_selection_access<int_with_parent>);
  EXPECT_THROW(s.end(), xmol::utils::dead_selection_access<int_with_parent>);
...
TEST_F(ConSelTests, selection_indexing) {
  ...
  EXPECT_NO_THROW(s[-2]);
  EXPECT_NO_THROW(s[-3]);
  EXPECT_THROW(s[-4], xmol::utils::selection_out_of_range<int_with_parent>);
  EXPECT_NO_THROW(s[0]);
  EXPECT_NO_THROW(s[1]);
  EXPECT_NO_THROW(s[2]);
  EXPECT_THROW(s[3], xmol::utils::selection_out_of_range<int_with_parent>);

  EXPECT_NO_THROW(s2[-1]);
  EXPECT_NO_THROW(s2[-2]);
  EXPECT_NO_THROW(s2[-3]);
  EXPECT_NO_THROW(s2[-4]);
  EXPECT_THROW(s2[-5], xmol::utils::selection_out_of_range<int_with_parent>);
  EXPECT_NO_THROW(s2[0]);
  EXPECT_NO_THROW(s2[1]);
  EXPECT_NO_THROW(s2[2]);
  EXPECT_NO_THROW(s2[3]);
  EXPECT_THROW(s2[4], xmol::utils::selection_out_of_range<int_with_parent>);
}
...
TEST_F(ConSelTests, access_dead_selection_throws) {
  ...
  ASSERT_EQ(c.size(), 0);
  ASSERT_EQ(s.size(), n);
  EXPECT_THROW(s[0], xmol::utils::deleted_element_access<int_with_parent>);
  EXPECT_THROW(*s.begin(), xmol::utils::deleted_element_access<int_with_parent>);
  EXPECT_THROW([&]{auto x = s.begin()->value;static_cast<void>(x);}(), xmol::utils::deleted_element_access<int_with_parent>);
}
