...
void DatTrajectoryPortion::close()  {
  m_stream->close();
}

bool DatTrajectoryPortion::match(const xmol::polymer::AtomSelection& atoms) const {
  return m_reader->match(atoms);
}
void DatTrajectoryPortion::set_coordinates(xmol::polymer::frameIndex_t frameIndex,
    const std::vector<std::pair<int, xmol::polymer::Atom*>>& index_atoms)
{
//  return m_reader->set_frame(frameIndex,)
  throw std::runtime_error("Not implemented");
}
void DatTrajectoryPortion::set_coordinates(xmol::polymer::frameIndex_t frameIndex, xmol::polymer::AtomSelection& atoms)
{
  if (!m_stream->is_open()){
